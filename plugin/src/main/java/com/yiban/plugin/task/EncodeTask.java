...
public class EncodeTask extends DefaultTask {

    protected final Project mProject;
    protected final EncodeExt mExt;

    @Inject
    public EncodeTask(Project project, EncodeExt ext) {
        this.mProject = project;
        this.mExt = ext;
        setGroup(YibanPlugin.group);
        setDescription("AES 加密Task");
    }
    ...
    public void taskAction() {
        ...
        boolean encodeConfigFile = findDecodeConfigFile();
        if (!encodeConfigFile) {
            return;
        }
    ...
    private void writeFile(String fileContent) {
        System.out.println("开始写入加密文件 content = " + fileContent);
        try {
    ...
    private String readFileContent() {
        ...
        try {
            ...
            while (keys.hasNext()) {
                ...
                System.out.println("加密之前 key =" + key + " value = " + value);
                value = EncryptionHelper.encodeToBase64String(value);
                System.out.println("加密之后 key =" + key + " value = " + value);
    ...
    private boolean findDecodeConfigFile() {

        File encodeFile = new File(mExt.getDecodePath(), mExt.getDecodeFileName());
        boolean createSuccess = false;
        try {
            if (!encodeFile.exists()) {
                System.out.println(String.format(Locale.CHINESE, "%s 加密配置文件不存在, 开始创建", mExt.getDecodeFileName()));
                createSuccess = encodeFile.createNewFile();
            } else {
                System.out.println(String.format(Locale.CHINESE, "找到%s加密配置文件", mExt.getDecodeFileName()));
                createSuccess = true;
            }
        } catch (IOException e) {
            System.out.println(String.format(Locale.CHINESE, "%s 加密配置文件不存在, 创建失败", mExt.getDecodeFileName()));
        }
        return createSuccess;
    }

    private boolean checkFile() {
        String dir = mExt.getDecodePath();
